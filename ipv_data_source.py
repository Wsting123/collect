...
class ipv_data_source:
	...
	def __init__(self, ip):
		...
		self.p_press_art_sys=0
		self.p_press_art_dias=0
		self.p_press_art_mean=0
	...
	def get_vital_signs(self):
		...
		# 心率
		if self.p_ecg_pulse > 0:
			ret_val.append(["心率", self.p_ecg_pulse])
		
		# PVC计数和率
		if self.p_pvc_count > 0:
			ret_val.append(["PVC计数", self.p_pvc_count])
		if self.p_pvc_rate > 0:
			ret_val.append(["PVC率", self.p_pvc_rate])
		
		# 无创血压
		if self.p_nbp_sys > 0:
			ret_val.append(["无创收缩压", self.p_nbp_sys])
		if self.p_nbp_dias > 0:
			ret_val.append(["无创舒张压", self.p_nbp_dias])
		if self.p_nbp_mean > 0:
			ret_val.append(["无创平均压", self.p_nbp_mean])
		if self.p_nbp_pulse > 0:
			ret_val.append(["无创脉搏", self.p_nbp_pulse])
		...
		if(self.p_nbp_time[0]!="")and(self.p_nbp_time[1]!=""):
			...
			ret_val.append(["无创血压时间", temp_time])
		
		# 有创血压
		if self.p_press_art_sys > 0:
			ret_val.append(["有创收缩压", self.p_press_art_sys])
		if self.p_press_art_dias > 0:
			ret_val.append(["有创舒张压", self.p_press_art_dias])
		if self.p_press_art_mean > 0:
			ret_val.append(["有创平均压", self.p_press_art_mean])
		
		# 呼吸频率
		if self.p_resp_rate > 0:
			ret_val.append(["呼吸频率", self.p_resp_rate])
		
		# 体温
		if self.p_temp > 0:
			ret_val.append(["体温", self.p_temp])
		
		# SpO2
		if self.p_spo2 > 0:
			ret_val.append(["血氧饱和度", self.p_spo2])
		if self.p_spo2_pulse > 0:
			ret_val.append(["血氧脉搏", self.p_spo2_pulse])
		
		# 未处理的physio_id
		if self.p_other_vitals:
			ret_val.append(["其他体征", ",".join(self.p_other_vitals)])
		
		# 时间信息
		ret_val.append(["连接开始后相对时间: 测量", self.p_timestamp])
		ret_val.append(["设备开机到连接开始的相对时间", self.monitor_rel_time])
		...
		if(self.monitor_abs_time[0]!="")and(self.monitor_abs_time[1]!=""):
			...
			temp_time="1990-01-01 00:00:00"
		
		ret_val.append(["连接开始时间", temp_time])
	...
	def extract_physoi_id(self, p_id, observ_val, handle_id):
		...
		# PVC计数
			...
			self.p_pvc_count=observ_val
		# PVC率
		elif p_id==16994:
			self.p_pvc_rate=observ_val
		# 无创血压
		elif p_id==18965:
		...
		# 有创血压
			...
			self.p_press_art_sys=observ_val
		elif p_id==18948:
			self.p_press_art_dias=observ_val
		elif p_id==18949:
			self.p_press_art_mean=observ_val
		# 呼吸频率
		elif p_id==20490:
		...
		# 未处理的physio_id
			...
			self.p_other_vitals.append(f"{p_id}:{observ_val}")
